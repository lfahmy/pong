...
class Field {
  ...
  detectWallBounce() {
    ...
    if(this.ball.rawY + this.ball.radius >= 509 || this.ball.rawY - this.ball.radius <= 91){
      //console.log("hit ceil/floor wall");
  ...
  getSpin(paddle) {
    ...
    console.log("paddle", paddle.type, "xSpin:", xSpin, "ySpin:", ySpin);
    this.ball.xSpin += xSpin;
    this.ball.ySpin += ySpin;
    console.log("ball -- xSpin:", this.ball.xSpin, "ySpin:", this.ball.ySpin);
  }

  hitBall(paddle) {
    if (paddle.hit(this.ball) && this.ball.fill === "gradient") {
      this.getSpin(paddle);
