...
import { renderWinningScore, playing, notPlaying } from './render';
import './main.css';
...
const connect = () => (
  connectedPromise.then(() => {
    ...
    socket.on(Constants.MSG_TYPES.WINNING_SCORE, renderWinningScore);
...
Promise.all([
  ...
  usernameInput.onkeypress = (event) => {
    // press enter
    if(event.charCode == 13) join();
  };
});


export const updateMouse = throttle(5, e => {
